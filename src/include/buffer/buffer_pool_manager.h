...
class BufferPoolManager {
 public:
  ...
  Page *FetchPage(page_id_t page_id, bufferpool_callback_fn callback = nullptr) {
    ...
    auto *result = FetchPageImp(page_id);
  ...
  bool UnpinPage(page_id_t page_id, bool is_dirty, bufferpool_callback_fn callback = nullptr) {
    GradingCallback(callback, CallbackType::BEFORE, page_id);
    auto result = UnpinPageImp(page_id, is_dirty);
    GradingCallback(callback, CallbackType::AFTER, page_id);
    return result;
  }
  ...
  bool FlushPage(page_id_t page_id, bufferpool_callback_fn callback = nullptr) {
    GradingCallback(callback, CallbackType::BEFORE, page_id);
    auto result = FlushPageImp(page_id);
    GradingCallback(callback, CallbackType::AFTER, page_id);
    return result;
  }
  ...
  Page *NewPage(page_id_t *page_id, bufferpool_callback_fn callback = nullptr) {
    ...
    auto *result = NewPageImp(page_id);
    GradingCallback(callback, CallbackType::AFTER, *page_id);
    return result;
  }
  ...
  bool DeletePage(page_id_t page_id, bufferpool_callback_fn callback = nullptr) {
    GradingCallback(callback, CallbackType::BEFORE, page_id);
    auto result = DeletePageImp(page_id);
    GradingCallback(callback, CallbackType::AFTER, page_id);
    return result;
  }
 ...
 protected:
   ...
   */
  virtual Page *FetchPageImp(page_id_t page_id) = 0;
   ...
   */
  virtual bool UnpinPageImp(page_id_t page_id, bool is_dirty) = 0;
   ...
   */
  virtual bool FlushPageImp(page_id_t page_id) = 0;
   ...
   */
  virtual Page *NewPageImp(page_id_t *page_id) = 0;
   ...
   */
  virtual bool DeletePageImp(page_id_t page_id) = 0;

  /**
   * Flushes all the pages in the buffer pool to disk.
   */
  virtual void FlushAllPagesImp() = 0;
};
}  // namespace bustub
